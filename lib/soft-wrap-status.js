...
export default class SoftWrapStatus {
   ...
   */
  constructor (editor = atom.workspace.getActiveTextEditor()) {
    this.editor = editor
    this.view = new SoftWrapStatusComponent(this)
    this.workspaceSubscription = atom.workspace.onDidChangeActivePaneItem(() => {
   ...
   *
   * @return {Boolean} `true` if the soft wrap indicator should be displayed, `false` otherwise.
   */
   ...
   * enabled.
   *
   * @return {Boolean} `true` if the indicator should be lit, `false` otherwise.
   */
  shouldBeLit () {
